...
First we define a generating function for the model. Note that a second set of parameters is included that reproduces another version of the system from the same text.
...
Now we generate a realization of the model.

```{r run model}
block_huisman <- gen_Huisman(200)
block_huisman <- block_huisman %>%
  mutate(regime=as.numeric(N2 > N4))
```

To see visualize how the different variables "see" and distort the native attractor, we define 3 different immersions of the data.
